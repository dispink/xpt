"""
Models architecture follows MAE's asymmtrical encoder-decoder structure,
but with linear layers instead of transformer blocks.
"""
...
class SimpleLinear(nn.Module):
    def __init__(self, spe_size=2048, patch_size=16, mask_ratio=0.75,
                 embed_dim=1024, decoder_embed_dim=512, norm_pix_loss=False):
        super().__init__()
        # --------------------------------------------------------------------------
        # Linear version of encoder specifics
        ...
        # --------------------------------------------------------------------------
        # Linear version of decoder specifics 
        self.decoder_embed = nn.Linear(embed_dim, decoder_embed_dim, bias=True)
        self.decoder = nn.Sequential(
            nn.Linear(decoder_embed_dim, 256, bias=True),
            nn.ReLU(),
            nn.Linear(256, 512, bias=True),
            nn.ReLU(),
            nn.Linear(512, decoder_embed_dim, bias=True),
            nn.ReLU()
        )
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.patch_embed.num_patches + 1, decoder_embed_dim), requires_grad=False)  # fixed sin-cos embedding
        self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.patch_embed.num_patches + 1, decoder_embed_dim), requires_grad=False)  # fixed sin-cos embedding
